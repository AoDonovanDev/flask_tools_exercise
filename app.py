...
def question(current):
    ...
    if current <= 3:
        ...
        current_q = current + 1
        button_content = 'next'
        return render_template('question.html', question = satisfaction_survey.questions[current].question, choices = satisfaction_survey.questions[current].choices, button = button_content, current = current_q, method = frm_method)
    return render_template('thanks.html', button = "return to start", current = '/', answers = responses)